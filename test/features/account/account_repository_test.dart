...
void main() {
  ...
  test('create() повинен повернути Account та очистити кеш при успішному створенні (201)', () async {
    // ARRANGE
    const accountId = 1;
    const familyId = 11;
  ...
  test('update() повинен відправити PUT запит та очистити кеш', () async {
    // 1. ARRANGE
    const accountId = 1;
    const familyId = 11;
    ...
    // 3. ASSERT
    expect(result, isA<Account>());
    verify(() => mockApi.put(path: 'accounts/$accountId', data: any(named: 'data'))).called(1);
  ...
  test('delete() повинен відправити DELETE запит та очистити кеш', () async {
    // 1. ARRANGE
    const accountId = 1;
    const familyId = 11;
    ...
    // 3. ASSERT
    verify(() => mockApi.delete(path: 'accounts/$accountId')).called(1);
    verify(() => mockCache.remove(namespace: 'accounts', key: 'user_accounts_family_$familyId')).called(1);
  });
}
